...
- `web-server.ts` serves the local browser UI and JSON API; static assets live in `web/`.
- `generate-sql.ts` owns the live OpenRouter agent construction shared by every interface.
...
- `test/web-server.test.ts` covers HTTP routes with a temporary SQLite database and injected generator.
